...
locals {
  ...
  // adding 000 for replacing later
  suffix             = join("-", [var.purpose, var.environment, var.region, "00"])
  suffix_unique      = join("-", concat([var.purpose, var.environment, var.region], [local.random], ["00"]))
  suffix_safe        = lower(join("", [var.purpose, var.environment, var.region, "00"]))
  suffix_unique_safe = lower(join("", concat([var.purpose, var.environment, var.region], [local.random], ["00"])))
